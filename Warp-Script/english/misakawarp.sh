...
install_wgcf(){
    ...
    if [[ $vpsvirt =~ lxc|openvz ]]; then
        if [[ $main -lt 5 ]] || [[ $minor -lt 6 ]]; then
            wget -N --no-check-certificate https://raw.githubusercontent.com/Clown1i/Misaka-Script/main/Warp-Script/files/wireguard-go -O /usr/bin/wireguard-go
            chmod +x /usr/bin/wireguard-go
        fi
    fi
    if [[ $vpsvirt == zvm ]]; then
        wget -N --no-check-certificate https://raw.githubusercontent.com/Clown1i/Misaka-Script/main/Warp-Script/files/wireguard-go-s390x -O /usr/bin/wireguard-go
        chmod +x /usr/bin/wireguard-go
    fi
